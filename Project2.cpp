...
main( int argc, char *argv[ ] )
{
	...
	fprintf( stderr, "This computer has %d processors.\n", numProcessors );
	...
	for( int n = 0; n < NUMTRIES; n++ )
	{
		...
		for( int t = 0; t < NUMSTEPS; t++ )
		{
			...
			for( int i = 0; i < NUMBODIES; i++ )
			{
				...
				float f = 0.;
				Body *bi = &Bodies[i];
				#pragma omp parallel for reduction(+:fx,fy,fz),private(f), schedule(static)
	...
	0.5*az*TIMESTEP*TIMESTEP;
		...
		double performance = ((double) (NUMBODIES*NUMBODIES*NUMSTEPS)/(time1-time0)/1000000.);
